...
export default function ChatMCQ({ mcq, onSubmit, disabled }: ChatMCQProps) {
  ...
  return (
    <div className="mt-2 p-3 bg-[#F5F5F7] rounded-lg space-y-3">
        ...
        {mcq.options.map((option, index) => (
          ...
          >
            <input
              ...
              className="mt-0.5 w-4 h-4 rounded border-gray-300 text-[#1A1A2E] focus:ring-[#1A1A2E] accent-[#1A1A2E]"
            />
            {option.isOther ? (
              <div className="flex-1 flex items-center gap-2">
                ...
                <input
                  ...
                  className="flex-1 px-2 py-1 text-sm rounded border border-gray-200 bg-white text-[#1A1A2E] placeholder:text-[#4A4A6A]/40 focus:outline-none focus:ring-1 focus:ring-[#1A1A2E]/30 disabled:opacity-50"
                />
              </div>
            ) : (
      ...
      {!submitted && (
        <button
          ...
          className="w-full py-2 text-sm font-medium rounded-full bg-[#1A1A2E] text-white hover:bg-[#2A2A4E] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
          Confirmer
        </button>
      )}
